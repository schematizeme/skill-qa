...
7. **Gate de CI trava o merge e NÃO se desliga "por enquanto".** Fail no `summary.json`, a11y vermelho
...
8. **Q.A. é parte da DoD e mora no archive.** Todo plano/resultado (`summary.json` + relatório) em
...
- **Gates:** PR (smoke+unit+a11y+lint), pré-deploy (`all`+visual+contrato), nightly (`full`+chaos+
  mutation). `summary.json` é o que o gate lê.
