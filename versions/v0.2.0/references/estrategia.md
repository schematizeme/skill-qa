...
| **property-based** | invariantes valem pra **todo** input, não só os do exemplo | unit + domínio |
| **mutation** | os testes **pegam** um bug injetado (mede o que verificam) | domínio crítico |
...
- PR check roda o rápido (smoke + unit afetado + a11y + lint) em minutos; pré-deploy roda o `all`;
  nightly roda o `full` (inclui chaos/mutation/carga). Detalhe da matriz em `execucao.md`.
