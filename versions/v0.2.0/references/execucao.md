...
Q.A. é a disciplina que **mais** dispara efeito externo por acidente: o laço que cria contas dispara um
OTP por conta, o teste de carga multiplica isso por mil, e o seed roda de novo toda noite. O incidente
de referência da casa — **>5.000 e-mails reais** para endereços sintéticos, reputação de IP e domínio
queimada, utilidade **zero** — nasceu assim. A normativa completa (DNS, guard, 4 camadas, exceção por
ADR, runbook de contenção) é da **`schematize-engineering` → `references/efeitos-externos.md`**; aqui
está o **recorte de Q.A.**: como o test kit é montado pra que isso seja **impossível em silêncio**.

**Regra:** efeito externo em teste é **sinkado por construção**, e o teste **lê do sink**. Ver
`categorias.md` §9 pra como **assertar** e-mail/notificação sem enviar.

### 5.1 Endereçamento de seeds, personas e fixtures

**MUST**
- Forma canônica: **`<papel>+<run-id>-<n>@test.<domain>`**. O `<run-id>` é o **mesmo** id do diretório
  de log da execução (`test-<YYYY-MM-DD>-<HHMMSS-pid>`, §2) — dá rastro direto de "qual run gerou este
  endereço" sem inventar domínio novo.
- O `test.<domain>` é **rota nula**: null MX (RFC 7505, `MX 0 .`) + SPF `v=spf1 -all` + DMARC
  `p=reject`. Sem DNS próprio, use TLD reservado: `.test`, `.invalid`, `.example`.
- Telefone/push seguem o mesmo princípio: **magic numbers** do provedor (sandbox) e token de projeto
  de teste — nunca um número/dispositivo real.

| Papel | Endereço de teste | Vetado |
|---|---|---|
| `superadmin` | `superadmin+r7f3a-1@test.<domain>` | qualquer caixa real |
| `tenant_admin_A` | `admin-a+r7f3a-1@test.<domain>` | domínio do cliente/parceiro |
| `normal_user` (carga, n=1..N) | `user+r7f3a-<n>@test.<domain>` | `@gmail.com`, `@hotmail.com`, … |
| dono/equipe "só pra ver chegando" | — **não existe** | **e-mail de pessoa real, inclusive o seu** |

**VETADO em fixture/seed/persona/factory/demo/carga:** `@gmail.com`/`@hotmail.com`/`@outlook.com`/
`@yahoo.com`/`@icloud.com`, domínio de terceiro ou do cliente, e-mail de **pessoa real (inclusive o
seu)**, o **domínio de produção** e endereços sem TLD (`@localhost`, `@teste` — alguns MTAs tentam
